...
#define TOTAL_BLOCKS 40UL//16384UL
...
__global__ void md5Crack(uint8_t wordLength, char* charsetWord, uint32_t hash01, uint32_t a0, uint32_t b0, uint32_t c0, uint32_t d0){
  ...
  for(uint32_t hash = 0; hash < HASHES_PER_KERNEL; hash++){
    ...
    md5Hash((unsigned char*)threadTextWord, threadWordLength, &threadHash01, &threadHash02, &threadHash03, &threadHash04, a0, b0, c0, d0); 

    if((threadHash01 & 0xFFFFFF) == hash01){
...
int main(int argc, char* argv[]){
  /* Check arguments */
  if(argc != 3){
    std::cout << argv[0] << " <salt> <target>" << std::endl;
    return -1;
  }
  
  uint32_t threadHash01, threadHash02, threadHash03, threadHash04;
  md5Hash_salt((unsigned char*)argv[1], 64, &threadHash01, &threadHash02, &threadHash03, &threadHash04);
  ...
  uint32_t hash_inp = strtol(argv[2], NULL, 16);  
  ...
  while(true){
    ...
    for(int device = 0; device < devices; device++){
      ...
      /* Start kernel */
      md5Crack<<<TOTAL_BLOCKS, TOTAL_THREADS>>>(g_wordLength, words[device], Hash, threadHash01, threadHash02, threadHash03, threadHash04);
    ...
    for(int device = 0; device < devices; device++){
      ...
      /* Copy result */
      ERROR_CHECK(cudaMemcpyFromSymbol(g_cracked, g_deviceCracked, CONST_WORD_LIMIT, 0, cudaMemcpyDeviceToHost)); 
      
      /* Check result */
      if(found = *g_cracked != 0){     
        std::cout << g_cracked << std::endl; 
        break;
      }
    }
